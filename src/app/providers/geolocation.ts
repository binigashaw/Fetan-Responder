...
import { DeviceService, GetConfigResultData, GpsLocation, SaveUnitLocationInput, UnitLocationService } from '@resgrid/ngx-resgridlib';
...
export class GeolocationProvider {
	...
	constructor(private loader: LazyGoogleMapsLoader, private platform: Platform, private storageProvider: StorageProvider, private deviceService: DeviceService, private unitLocationService: UnitLocationService, private store: Store<HomeState>, private http: HttpClient) {
	...
	public async getLocation(): Promise<GeoLocation> {
		...
		if (position && position.coords.latitude != 0 && position.coords.longitude && position.coords.longitude != 0) {
			return new GeoLocation(position.coords.latitude, position.coords.longitude);
		}
	...
	public async startTracking() {
		if (await this.storageProvider.getEnableRealtimeGeolocation()) {
				...
				this.watchPositionId = await Geolocation.watchPosition(
					...
					(position, err) => {
						if (position) {
							...
							this.store.dispatch(new HomeActions.GeolocationLocationUpdate(location));
						}
					}
				);
			}
		...
		return this.geocode({ address: address }).pipe(
			map((data) => {
				if (data && data.length > 0) {
					return new GeoLocation(data[0].geometry.location.lat(), data[0].geometry.location.lng());
				}
	...
	public getCoordinatesFromW3W(w3w: string): Observable<GeoLocation> {
		return this.http.get(`https://api.what3words.com/v3/convert-to-coordinates?words=${w3w}&key=${this.w3wKey}`).pipe(
			map((data: any) => {
